    ...
    <Box sx={{ minHeight: '100vh', background: T.bg, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
      ...
      <Box sx={{
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <Typography sx={{ color: T.white, fontWeight: 700, fontSize: '0.95rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Student Portal
          </Typography>
        </Box>
      ...
      <Box sx={{
        ...
        <Box sx={{ maxWidth: 1200, mx: 'auto', position: 'relative' }}>
          <Box
            ...
            <Typography sx={{ fontSize: '0.8rem', fontWeight: 600, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>Dashboard</Typography>
          </Box>

          <Typography sx={{ color: T.gold, fontSize: '0.72rem', fontWeight: 600, letterSpacing: '0.14em', textTransform: 'uppercase', mb: 0.75, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            Weekly Schedule
          </Typography>
          <Typography sx={{ color: T.white, fontSize: { xs: '1.7rem', md: '2.2rem' }, fontWeight: 700, fontFamily: "'Fraunces', serif", letterSpacing: '-0.02em', lineHeight: 1.2, mb: 1.5 }}>
            My Timetable
          </Typography>
          ...
          {data?.class && (
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Box sx={{ px: 1.5, py: 0.4, borderRadius: '20px', background: 'rgba(255,255,255,0.1)', border: '1px solid rgba(255,255,255,0.15)', display: 'flex', alignItems: 'center', gap: 0.75 }}>
                ...
                <Typography sx={{ color: 'rgba(255,255,255,0.85)', fontSize: '0.78rem', fontWeight: 600, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  Class {data.class.name}
                </Typography>
              </Box>
              {data.class.stream && (
                <Box sx={{ px: 1.5, py: 0.4, borderRadius: '20px', background: `${T.gold}22`, border: `1px solid ${T.gold}44` }}>
                  <Typography sx={{ color: T.goldLight, fontSize: '0.78rem', fontWeight: 600, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                    {data.class.stream}
                  </Typography>
                </Box>
              )}
              <Box sx={{ px: 1.5, py: 0.4, borderRadius: '20px', background: 'rgba(255,255,255,0.08)', border: '1px solid rgba(255,255,255,0.12)' }}>
                <Typography sx={{ color: 'rgba(255,255,255,0.65)', fontSize: '0.78rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                  Today: {today}
                </Typography>
              </Box>
            </Box>
      ...
      <Box sx={{ px: { xs: 1, md: 5 }, py: 4, maxWidth: 1280, mx: 'auto' }}>
        ...
        {error && (
          <Box sx={{ textAlign: 'center', py: 8 }}>
            <Typography sx={{ color: T.muted, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>{error}</Typography>
          </Box>
        )}
        ...
        {!loading && !error && data?.class && data.periods.length === 0 && (
          <Box sx={{
            ...
            <Typography sx={{ fontWeight: 700, fontSize: '1.1rem', color: T.text, mb: 0.75, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              Timetable coming soon
            </Typography>
            <Typography sx={{ color: T.muted, fontSize: '0.88rem', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
              You're enrolled in {data.class.name}, but the school hasn't set up class periods yet.
              <br />Check back soon.
            </Typography>
          </Box>
        )}
            ...
            {(() => {
              ...
              return (
                <Box sx={{ px: 3, py: 2, borderBottom: `1px solid ${T.border}`, background: '#FAFBFF', display: 'flex', flexWrap: 'wrap', gap: 1, alignItems: 'center' }}>
                  <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: T.muted, textTransform: 'uppercase', letterSpacing: '0.08em', mr: 0.5, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                    Subjects:
                  </Typography>
                  {subjects.map(sub => {
                    ...
                    return (
                      <Box key={sub} sx={{ px: 1.25, py: 0.25, borderRadius: '6px', background: col.bg, border: `1px solid ${col.border}` }}>
                        <Typography sx={{ fontSize: '0.7rem', fontWeight: 700, color: col.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                          {sub}
                        </Typography>
                      </Box>
                    );
              ...
              <Box sx={{ minWidth: 680 }}>
                ...
                <Box sx={{ display: 'grid', gridTemplateColumns: '130px repeat(5, 1fr)', borderBottom: `2px solid ${T.navy}` }}>
                  ...
                  <Box sx={{ px: 2, py: 1.5, background: T.navy }}>
                    <Typography sx={{ fontSize: '0.65rem', fontWeight: 700, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                      Period
                    </Typography>
                  </Box>
                  {/* Day header cells */}
                  {DAYS.map(day => {
                    ...
                    return (
                      <Box key={day} sx={{
                        ...
                        {isToday && (
                          <Typography sx={{ fontSize: '0.6rem', fontWeight: 700, color: T.navy, fontFamily: "'Plus Jakarta Sans', sans-serif", opacity: 0.7 }}>
                            Today
                          </Typography>
                        )}
                      </Box>
                  ...
                  return (
                    <Box key={period.periodNumber} sx={{
                      ...
                      <Box sx={{
                        ...
                        <Typography sx={{ fontSize: '0.65rem', color: isBreak ? T.breakText : T.muted, fontFamily: "'Plus Jakarta Sans', sans-serif", mt: 0.2 }}>
                      ...
                      {DAYS.map(day => {
                        ...
                        if (isBreak) return (
                          <Box key={day} sx={{
                            ...
                            <Typography sx={{ fontSize: '0.68rem', color: T.breakText, fontStyle: 'italic', fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
                              {period.name}
                            </Typography>
                          </Box>
                        );
                        ...
                        if (slot) {
                          ...
                          return (
                            <Box key={day} sx={{
                              ...
                              <Typography sx={{ fontSize: '0.66rem', color: col.text, opacity: 0.75, fontFamily: "'Plus Jakarta Sans', sans-serif", mt: 0.25 }}>
