...
/* ════════════════════════════════════════════════════════════
   QUIZ TAB
════════════════════════════════════════════════════════════ */
export default function QuizTab({ onGoToMaterials }) {
  // -- view: 'quizzes' | 'generate' | 'review' | 'results'
  const [view,        setView]        = useState('quizzes');
  const [quizzes,     setQuizzes]     = useState([]);
  const [materials,   setMaterials]   = useState([]); // read-only here — managed on the Learning Materials tab
        ...
        <Box>
          ...
          : quizzes.length === 0 ? (
            <Card sx={{ textAlign: 'center', py: 8 }}>
              ...
              <Typography sx={{ fontSize: '0.82rem', color: C.muted, fontFamily: "'IBM Plex Sans', sans-serif", mt: 0.5 }}>Add learning materials on the Learning Materials tab, then use Generate Quiz to create your first quiz.</Typography>
              <Button onClick={onGoToMaterials} variant="contained" sx={{ mt: 2.5, background: C.purple, textTransform: 'none', fontWeight: 700, borderRadius: '8px', fontFamily: "'IBM Plex Sans', sans-serif" }}>
                ...
                {materials.length === 0 ? (
                  <Box sx={{ p: 2, border: `1px solid ${C.border}`, borderRadius: '8px', textAlign: 'center' }}>
                    <Typography sx={{ fontSize: '0.8rem', color: C.muted, fontFamily: "'IBM Plex Sans', sans-serif" }}>
                      No materials yet — <Button size="small" onClick={onGoToMaterials} sx={{ textTransform: 'none', fontWeight: 700, color: C.purple, fontFamily: "'IBM Plex Sans', sans-serif", p: 0, minWidth: 0 }}>add some on the Learning Materials tab</Button>
                    </Typography>
                  </Box>
                ) : (
