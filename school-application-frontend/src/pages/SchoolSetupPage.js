...
/* ══════════════════════════════════════════════════════════════════
   ...
   These SECTION components live at module scope, not nested inside
   SchoolSetupPage: a component defined inside another component's
   function body gets a brand-new function identity every render, so
   React treats it as a different component type and remounts it —
   wiping any in-progress local state (a half-filled form, unsaved
   edits) whenever SchoolSetupPage re-renders for any reason, even one
   as unrelated as the toast Snackbar auto-closing. Data that used to
   come from closure now comes in as props instead.
══════════════════════════════════════════════════════════════════ */
const AcademicYearSection = ({ academicYears, toast, fetchYears, fetchSummary }) => {
...
/* ══════════════════════════════════════════════════════════════════
   TERMS SECTION
══════════════════════════════════════════════════════════════════ */
const TermsSection = ({ terms, summary, toast, fetchTerms, fetchSummary }) => {
...
/* ══════════════════════════════════════════════════════════════════
   PERIODS SECTION
══════════════════════════════════════════════════════════════════ */
const PeriodsSection = ({ periods, toast, fetchPeriods, fetchSummary }) => {
...
/* ══════════════════════════════════════════════════════════════════
   SUBJECTS SECTION
══════════════════════════════════════════════════════════════════ */
const SubjectsSection = ({ subjects, summary, toast, fetchSubjects, fetchSummary }) => {
  ...
  const fetchNational = useCallback(async (grade, stream) => {
  ...
  }, [addGrade, addStream, fetchNational]);
...
/* ══════════════════════════════════════════════════════════════════
   CLASSES SECTION
══════════════════════════════════════════════════════════════════ */
const ClassesSection = ({ classes, summary, toast, fetchClasses, fetchSummary }) => {
  ...
  return (
    <Box sx={{ minHeight: '100vh', background: '#F0F4F8', fontFamily: "'IBM Plex Sans', sans-serif" }}>
      ...
      <Container maxWidth="lg" sx={{ py: 4 }}>
        ...
        {activeStep === 'year' && (
          <AcademicYearSection
            academicYears={academicYears} toast={toast}
            fetchYears={fetchYears} fetchSummary={fetchSummary}
          />
        )}
        {activeStep === 'terms' && (
          <TermsSection
            terms={terms} summary={summary} toast={toast}
            fetchTerms={fetchTerms} fetchSummary={fetchSummary}
          />
        )}
        {activeStep === 'periods' && (
          <PeriodsSection
            periods={periods} toast={toast}
            fetchPeriods={fetchPeriods} fetchSummary={fetchSummary}
          />
        )}
        {activeStep === 'subjects' && (
          <SubjectsSection
            subjects={subjects} summary={summary} toast={toast}
            fetchSubjects={fetchSubjects} fetchSummary={fetchSummary}
          />
        )}
        {activeStep === 'classes' && (
          <ClassesSection
            classes={classes} summary={summary} toast={toast}
            fetchClasses={fetchClasses} fetchSummary={fetchSummary}
          />
        )}
