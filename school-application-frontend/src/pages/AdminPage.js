...
const AdminPage = () => {
  ...
  const getDocumentTypeName = type => {
    const map = { id: 'South African ID Copy', removal: 'School Removal Letter', gradeResult: 'Previous Grade Result', gradeReport: 'Grade Report', additional: 'Additional Document' };
    return map[type] || type || 'Unknown Document';
  };
  const getDocumentIcon     = type => ({ id: 'ID', removal: 'REM', gradeResult: 'GRD', gradeReport: 'RPT', additional: 'ADD' })[type] || 'DOC';
                      ...
                      <Stack spacing={1.25}>
                        {selectedApp.documents.map((doc, idx) => (
                          <Paper key={idx} variant="outlined" sx={{
                            ...
                            <Stack direction="row" alignItems="center" spacing={1.5}>
                              <Avatar variant="rounded" sx={{ bgcolor: '#eff6ff', color: 'primary.dark', fontSize: '0.72rem', fontWeight: 700, width: 40, height: 40 }}>
