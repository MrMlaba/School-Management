...
// ─────────────────────────────────────────────────────────────────────────────
//  HELPER — derive stream from subject + grade
// ─────────────────────────────────────────────────────────────────────────────
const deriveStream = (grade, subject) => {
  ...
  if (['science','physics','physical','life sciences','biology','chemistry','mathematics','maths','math'].some(k => s.includes(k))) return 'Science';
  if (['accounting','commerce','business','economics','bst'].some(k => s.includes(k))) return 'Commerce';
  if (['history','geography','humanities','dramatic','arts','religion','tourism','consumer'].some(k => s.includes(k))) return 'Humanities';
  return 'General';
};
...
//  Uses: a.school = school_name   AND   es.school_id = school_id
// ─────────────────────────────────────────────────────────────────────────────
router.get('/pending-enrollment', async (req, res) => {
  const schoolName = req.admin.school;    // e.g. "Green Valley High"
  const schoolId   = req.admin.schoolId;  // e.g. 3
...
//  Properly enrolled students from enrolled_students table.
//  Supports ?grade=10 and ?recent=1 query params.
// ─────────────────────────────────────────────────────────────────────────────
router.get('/enrolled-students', async (req, res) => {
  ...
  try {
    ...
    if (grade) {
      params.push(grade);
      query += ` AND es.grade = $${params.length}`;
    }

    if (recent === '1') {
      query += ` AND es.enrollment_date >= NOW() - INTERVAL '7 days'`;
    }
...
router.post('/enroll', async (req, res) => {
  ...
  try {
    ...
    // ── 1. Fetch application — use school name, not school_id ─────────────
    ...
    if (apps.length === 0) {
      ...
      return res.status(404).json({
        message: 'Application not found, does not belong to this school, or is not in accepted status.'
      });
    }
    ...
    const { rows: numRows } = await client.query(
      `SELECT generate_student_number($1) AS num`,
      [schoolId]
    );
    ...
    // ── 6. Upsert parent — dedup by phone within school ────────────────────
    if (app.parent_phone || app.parent_email) {
      ...
      if (!parentId) {
        ...
        const { rows: newParent } = await client.query(
          `INSERT INTO parents
             (school_id, first_name, last_name, phone, email, relationship, is_emergency_contact)
    ...
    res.status(201).json({
      ...
      student: {
        ...
        studentNumber,
        firstName:      enrolledStudent.first_name,
        lastName:       enrolledStudent.last_name,
        grade:          enrolledStudent.grade,
        stream:         enrolledStudent.stream,
    ...
    const { rows } = await pool.query(
      `SELECT
         (SELECT COUNT(*)
          ...
          WHERE school_id = $2 AND is_active = true)          AS "totalEnrolled",
         ...
         (SELECT COUNT(*)
          ...
          WHERE a.school = $1 AND a.status = 'accepted'
            AND NOT EXISTS (
              ...
              WHERE es.application_id = a.id AND es.school_id = $2
            ))                                                 AS "pendingEnrollment",

         (SELECT COUNT(*)
          FROM parents
          WHERE school_id = $2)                               AS "totalParents"`,
      [schoolName, schoolId]
    );
...
//  Dashboard widget — avoids conflict with eventsRoutes.js
// ─────────────────────────────────────────────────────────────
router.get('/upcoming-events', async (req, res) => {
 ...
 // ← this line already exists, don't duplicate it
